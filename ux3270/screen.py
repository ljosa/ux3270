...
class Screen:
    ...
    def render(self):
        ...
        if self.title:
            ...
            print(f"\033[1m{title_display}\033[0m", flush=True)
        ...
        for field in self.fields:
            ...
            if field.label:
                ...
                print(field.label + ":", end="", flush=True)
                
            # Display field
            self.move_cursor(field.row, field.col)
            if field.field_type == FieldType.READONLY:
                print(f"\033[2m{field.value}\033[0m", end="", flush=True)
            elif field.field_type == FieldType.PASSWORD:
                print("*" * len(field.value), end="", flush=True)
            else:
                print(field.value, end="", flush=True)
                
            # Show field placeholder
            remaining = field.length - len(field.value)
            if remaining > 0 and field.field_type != FieldType.READONLY:
                print("_" * remaining, end="", flush=True)
                
        # Display error message if present
        if self.error_message:
            self.move_cursor(self.get_screen_height() - 2, 0)
            print(f"\033[31m{self.error_message}\033[0m", end="", flush=True)
            
        # Display help text
        self.move_cursor(self.get_screen_height() - 1, 0)
    ...
    def get_input(self, field: Field) -> str:
        ...
        try:
            ...
            while True:
                ...
                print(display_value + "_" * (field.length - len(value)), end="", flush=True)
